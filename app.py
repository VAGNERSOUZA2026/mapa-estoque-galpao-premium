...
if pallet_param:
    ...
    vinhos_no_pallet = [v for v in st.session_state.estoque if v.get("pallet") == pallet_nome]
    ...
    if vinhos_no_pallet:
        ...
        for v in vinhos_no_pallet:
            st.markdown(f"""
                ...
                """, unsafe_allow_html=True)
            if v.get("foto"):
                st.image(base64.b64decode(v.get("foto")), caption="📸 Rótulo do Vinho", use_container_width=True)
                st.markdown("---")
    else:
...
if menu == "🔍 Buscar vinho":
    ...
    aba_texto, aba_foto = st.tabs(["🔎 Buscar por Texto / Pallet", "📸 Buscar por Foto do Rótulo"])
    
    with aba_texto:
        c_tipo, c_busca = st.columns([1, 2])
        with c_tipo:
            sub_op = st.selectbox("Filtrar por:", ["Por Nome", "Por Tipo", "Por Safra", "Por Pallet / Corredor"])
        with c_busca:
            termo = st.text_input("Digite o que procura:").strip().lower()
            
        if termo:
            resultados = [v for v in st.session_state.estoque if termo in str(v.get(sub_op.split()[-1].lower(), "")).lower() or termo in str(v.get("nome", "")).lower()]
            if not resultados:
                ...
                for v in resultados:
                    ...
                    if v.get("foto"):
                        with st.expander("🔍 Ver Foto do Rótulo", expanded=True):
                            st.image(base64.b64decode(v.get("foto")), use_container_width=True)
                    st.markdown("---")

    with aba_foto:
        foto_pesquisa = st.file_uploader("Selecione a foto da garrafa:", type=["jpg", "jpeg", "png"])
        if foto_pesquisa is not None:
            try:
                img_pesquisa = Image.open(foto_pesquisa)
                st.image(img_pesquisa, caption="Foto para Busca", width=280)
                ...
                if encontrados:
                    st.success(f"🎯 Encontrado(s) {len(encontrados)} resultado(s):")
    ...
    with st.form(f"form_cadastrar_{st.session_state.form_key}"):
        ...
        foto_upload = st.file_uploader("📸 Foto do Rótulo:", type=["jpg", "jpeg", "png"])
        btn_salvar = st.form_submit_button("✅ Salvar Vinho", use_container_width=True)
        ...
        if btn_salvar:
            ...
            if nome and tipo:
                ...
                novo_vinho = {
                    ...
                    "pallet": pallet_final, "lado": lado,
                    "caixa": caixa, "volume": volume, "foto": foto_b64
                }
    ...
    if st.session_state.estoque:
        opcoes = [f"{i+1}. {v.get('nome')} - {v.get('pallet')}" for i, v in enumerate(st.session_state.estoque)]
    ...
    if st.session_state.estoque:
        ...
        idx = st.selectbox("Escolha para apagar:", range(len(opcoes)), format_func=lambda x: opcoes[x])
    ...
    if arquivo_planilha is not None:
        try:
            if arquivo_planilha.name.endswith('.csv'):
                df_import = pd.read_csv(arquivo_planilha, sep=None, engine='python')
            else:
    ...
    link_pallet = f"{URL_APLICATIVO}/?pallet={pallet_encoded}&auth={SENHA_ACESSO}"
    ...
    with col_qr:
        st.image(url_qr, caption=pallet_alvo, width=220)
    with col_acoes:
        st.markdown("### 🖨️ Opções")
        st.markdown(f'<a href="javascript:window.print()" style="display:inline-block;width:100%;background-color:#581825;color:whi
